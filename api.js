...
const API = {
    ...
    RETRY_DELAY_MS: 600,
    /**
     * เวลารอสูงสุดของแต่ละครั้ง
     *
     * เดิมตั้งไว้ 8 / 13 / 19 วินาที โดยอิงว่าเซิร์ฟเวอร์ปกติตอบใน 3-6 วินาที
     * แต่วัดจริงวันที่ 15 ก.ย. 2569 ได้ว่า:
     *   พนักงานโหลดข้อมูล (~145 KB)  ใช้ 11.6 วินาที
     *   แอดมินโหลดข้อมูล  (~600 KB)  ใช้ 19-22 วินาที (ครั้งแรกหลัง deploy 56 วินาที)
     * รอบที่สำเร็จตอบกลับที่ 19.015 วินาที ช้ากว่าเพดานเดิมแค่ 15 มิลลิวินาที
     * หน้าเว็บจึงตัดทิ้งทุกครั้ง แอดมินทั้งหมดเข้าระบบไม่ได้
     *
     * ทำไมครั้งแรกต้องรอนาน ไม่ใช่ตัดเร็วแล้วลองใหม่:
     * การตัดที่ฝั่งเบราว์เซอร์ไม่ได้หยุดเซิร์ฟเวอร์ มันยังทำงานต่อจนจบ
     * แล้วการลองใหม่ก็สั่งให้ทำงานเต็มชุดอีกรอบ งานซ้อนกันทำให้รอบถัดไปช้าลงอีก
     *
     * ยังลองใหม่ได้เร็วเมื่อเซิร์ฟเวอร์ตอบหน้า error กลับมา (ไม่ต้องรอจนหมดเวลา)
     * รวมกรณีแย่ที่สุดที่เซิร์ฟเวอร์ไม่ตอบเลย ประมาณ 2 นาที
     */
    TIMEOUTS_MS: [30000, 45000, 60000],
    ...
    async _request(url, options, label, allowRetry = true) {
        ...
        for (let attempt = 1; attempt <= maxTries; attempt++) {
            ...
            /**
             * คำสั่งที่ลองซ้ำไม่ได้ (เช่น สร้างคำขอ) ได้ยิงแค่ครั้งเดียว จึงต้องใช้เวลารอยาวสุด
             * เดิมใช้ช่องแรก (8 วินาที) ขณะที่เซิร์ฟเวอร์ใช้ 11-22 วินาที
             * ผลคือหน้าเว็บขึ้นว่า "ส่งไม่สำเร็จ" ทั้งที่เซิร์ฟเวอร์บันทึกไปแล้ว
             * พนักงานเลยกดส่งซ้ำ แล้วได้คำขอซ้ำในชีต
             */
            const limit = maxTries === 1
                ? this.TIMEOUTS_MS[this.TIMEOUTS_MS.length - 1]
                : (this.TIMEOUTS_MS[attempt - 1] || this.TIMEOUTS_MS[this.TIMEOUTS_MS.length - 1]);
